...
| `@input` | `<!-- @input -->` | 位置占位符，请求时替换为最后一条用户消息的内容 |
...
`@input` 是一个**位置占位符**，可以在场景文件的任意位置插入。请求处理时，它会被替换为请求体中最后一条 `role: "user"` 消息的内容，并按当前切分策略流式输出。
...
任意用户消息都会被逐词回显。延迟由 `--default-delay` / `-d` CLI 参数控制。内置 `echo` 场景即此模式。
...
用户消息会被插入在"我来回答"和"总结"之间，形成完整的对话流。

### 注意事项

- `@input` 可以出现多次，每次都会展开为用户消息
- 用户消息为空时，该位置不输出任何内容（静默跳过）
- 指令前的 `@delay` 会应用到展开后的每个 chunk
- `@input` 支持所有切分策略（`word` / `sentence` / `char` / `line` / `paragraph`）
